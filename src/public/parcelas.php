...
<title>Pear - Parcelas</title>
<link href="cssPrincipal.css" rel="stylesheet" type="text/css" />
<script type="text/javascript">
	function envia(){
		window.opener.document.forms['frmgravavenda'].submit();
		self.close();
	}
</script>
</head>
		...
		if($_GET){
			...
			while($linha = mysql_fetch_array($resultado)){
				...
				?>
						<tr>
						<td>
							<input type="text" readonly="readonly" name="num" size="4" style="text-align:right" value="<?php echo $num; ?>" />
						</td>
						<td>
							<input type="text" readonly="readonly" name="datavcto" size="10" style="text-align:center" value="<?php echo $datavcto = date('d-m-Y', strtotime($datacaixa."+ ".$linha['dias']." day" ));?>" />
						</td>
						<td>
							<input type="text" readonly="readonly" name="valorparc" size="7" style="text-align:right" value="<?php echo number_format($valorfinal, 2) ?>" />
							<?php 
					...
					<td>
						<?php
							if(($num == $numlin) ){
								...
								?>
								<input type="text" readonly="readonly" name="valorparc" size="7" style="text-align:right" value="<?php echo $valorparc; ?>" />
								<?php
							}else{
								?>
								<input type="text" readonly="readonly" name="valorparc" size="7" style="text-align:right" value="<?php echo $valorparc; ?>" />

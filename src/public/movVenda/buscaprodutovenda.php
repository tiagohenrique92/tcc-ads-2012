<?php
	...
	if(isset($_GET['rdnomecodigo'])){
		if($_GET['rdnomecodigo'] == 'n'){
			$nome = $_GET['valbusca'];
			$where = " where nome like '%$nome%' and status = 'A' and idpro not in (select idpro from itemvenda where idvenda = $idvenda)";
		}else{
			$barras = $_GET['valbusca'];
			$where = " where barras = $barras and status = 'A' and idpro not in (select idpro from itemvenda where idvenda = $idvenda)";
		}
	}
	...
	function listarProduto($select){
		...
		while($produto = mysql_fetch_assoc($select)){
            ...
            <tr>
                ...
                <td width="70px" align="center"><?php echo $produto['precovenda']; ?></td>
                <td width="40px" align="center">
                	<a href="#" class="spriteIcons btnAdd" title="Selecionar" barras="<?php echo $produto['barras']?>" nomepro="<?php echo $produto['nome']?>" qtdepro="<?php echo $produto['qtde']?>" precopro="<?php echo $produto['precovenda']?>" idpro=<?php echo $produto['idpro']; ?>></a>
                </td>
            </tr>
            <?php
  		}

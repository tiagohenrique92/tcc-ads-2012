        	...
        	<?php
				if(!($_POST)){
                    ...
                    <legend>Buscar Cliente</legend>
                    	<form name="frmbuscacli" action="buscacliconta.php" method="post" onsubmit="return checarDadosObg(this.name)">
                        	<table>
                                <tr>
					...
					?>
                    <a style="text-decoration:none; color:#000000;" href="buscacliconta.php" title="Nova Busca">Nova Busca</a>
                    <?php
					$btnEnviar = $_POST['btnEnviar'];
					switch($btnEnviar){
						case "Buscar":
							$nome = strtoupper($_POST['nome']);
							$sql = "select idcli, nome, fone, celular from cliente where (nome like '%$nome%') and (idcli in (select idcli from venda where status = 'A')) order by nome";
